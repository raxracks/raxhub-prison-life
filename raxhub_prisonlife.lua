...
local mainWindow = library:AddWindow("raxHub - Prison Life")
local mapTab = mainWindow:AddTab("Map")
local playerTab = mainWindow:AddTab("Player")
local combatTab = mainWindow:AddTab("Combat")

mapTab:AddLabel("Teleports")

mapTab:AddButton("Criminal Spawn", function()
	game.Players.LocalPlayer.Character:MoveTo(Vector3.new(-920.963623, 95.3172913, 2138.39819))
end)

mapTab:AddButton("Criminal Base Spawn", function()
	game.Players.LocalPlayer.Character:MoveTo(Vector3.new(-975.404907, 109.321609, 2070.89038))
end)

mapTab:AddButton("Guard Spawn", function()
	game.Players.LocalPlayer.Character:MoveTo(Vector3.new(851.853882, 99.9900055, 2292.77686))
end)

mapTab:AddButton("Courtyard", function()
	game.Players.LocalPlayer.Character:MoveTo(Vector3.new(810.800354, 97.9999466, 2463.33667))
end)

mapTab:AddLabel("Misc")

mapTab:AddButton("Remove doors", function()
	for i, v in pairs(game.Workspace.Doors:GetChildren()) do
		v:Destroy()
	end
end)

local walkSpeedSlider = playerTab:AddSlider("Walkspeed", function(value)
	game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = value
end)

game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = originalWalkspeed
walkSpeedSlider:Set(game.Players.LocalPlayer.Character.Humanoid.WalkSpeed)

local jumpPowerSlider = playerTab:AddSlider("Jump power", function(value)
	game.Players.LocalPlayer.Character.Humanoid.JumpPower = value
end, {min = 0, max = 200})

game.Players.LocalPlayer.Character.Humanoid.JumpPower = (originalJumpPower / 2)
jumpPowerSlider:Set(game.Players.LocalPlayer.Character.Humanoid.JumpPower)

playerTab:AddSwitch("Infinite Stamina (unbelievably unreliable)", function(state)
...
combatTab:AddButton("Apply gun mod", function()
	for i, v in pairs(getGuns()) do
		local gunState = require(game.Players.LocalPlayer.Backpack[v.Name].GunStates)
		gunState["MaxAmmo"] = 999999999999999999
		gunState["CurrentAmmo"] = 999999999999999999
		gunState["FireRate"] = 0.00001
		gunState["AutoFire"] = true
		gunState["Range"] = 10000
		gunState["ReloadTime"] = 0
		gunState["Bullets"] = 10
	end

	if(getCurrentGun().Name == nil) then return end
	local gunState = require(game.Players.LocalPlayer.Backpack[getCurrentGun().Name].GunStates)
	gunState["MaxAmmo"] = 999999999999999999
	gunState["CurrentAmmo"] = 999999999999999999
	gunState["FireRate"] = 0.00001
	gunState["AutoFire"] = true
...
function getCurrentGun() 
	...
	for i, v in pairs(game.Players.LocalPlayer.Character:GetChildren()) do
		if v.Name == "M9" then
            weapon = v
		elseif v.Name == "AK-47" then
            weapon = v
        elseif v.Name == "Remington 870" then
            weapon = v
		end
	end
...
function findNearestPlayer()
    ...
    for i, v in pairs(game.Players:GetPlayers()) do
        if v and v.Character then
            local dist = v:DistanceFromCharacter(game.Players.LocalPlayer.Character.Torso.Position)
            if dist < lowest then
				if v.Name ~= game.Players.LocalPlayer.Name then
                	lowest = dist
                	nearestPlayer = v
...
while wait() do
	...
	for i, v in pairs(getGuns()) do
		if(not weaponDropdownItemTable[v.Name]) then
